...
function Medicaments() {
  ...
  const filteredMedicaments = useMemo(() => {
    return medicaments.filter(med => {
      ...
      const matchesFilter = filterGroupe === "" || med.groupe.toLowerCase().includes(filterGroupe.toLowerCase());
    ...
    <div className="p-4 sm:p-6 bg-gray-100 h-screen">
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        {loading ? (
          ...
          <div className="p-4 text-center text-gray-500">
            {filteredMedicaments.length === 0 && medicaments.length > 0 
          ...
          <>
            <div className="overflow-x-auto">
              <table className="w-full text-left min-w-[600px]">
                <thead className="bg-gray-100">
                  <tr>
                    <th className="px-3 sm:px-4 py-3 text-gray-700 font-bold text-lg sm:text-base">
                      Nom du médicament
                    </th>
                    <th className="px-3 sm:px-4 py-3 text-gray-700 font-bold text-lg sm:text-base">ID du médicament</th>
                    <th className="px-3 sm:px-4 py-3 text-gray-700 font-bold text-lg sm:text-base">Nom du groupe</th>
                    <th className="px-3 sm:px-4 py-3 text-gray-700 font-bold text-lg sm:text-base">Stock en quantité</th>
                    <th className="px-3 sm:px-4 py-3 text-gray-700 font-bold text-lg sm:text-base">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {currentMedicaments.map((med) => (
                    ...
                    >
                      <td className="px-3 sm:px-4 py-3 text-lg font-medium sm:text-base">
                        {med.nom}
                      </td>
                      <td className="px-3 sm:px-4 py-3 text-sm font-medium sm:text-base">{med.id}</td>
                      <td className="px-3 sm:px-4 py-3 text-sm font-medium sm:text-base">{med.groupe}</td>
                      <td className="px-3 sm:px-4 py-3 text-sm font-medium sm:text-base">{med.stock}</td>
                      <td
                        className="px-3 sm:px-4 py-3 text-gray-600 font-bold cursor-pointer hover:text-teal-700 text-sm sm:text-base"
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between px-3 sm:px-4 py-3 border-t bg-gray-50 gap-3">
              <p className="text-lg sm:text-sm text-gray-700 text-center sm:text-left">
                Affichage de {indexOfFirstItem + 1} à{" "}
                {Math.min(indexOfLastItem, filteredMedicaments.length)} résultats sur{" "}
